...
[![verify-tier](https://github.com/whoolm/makalah-ceritas-public/actions/workflows/verify-tier.yml/badge.svg)](https://github.com/whoolm/makalah-ceritas-public/actions/workflows/verify-tier.yml)
...
>
> Salinan publik dari `references/parafrase-tiers.md`. Isi tier (§1–§7) identik;
> tambahan khusus publik: badge build di atas + §8 Cara Berkontribusi di bawah.
...
1. Baca `docs/index.md` (quick start) dan `TASK_BREAKDOWN.md` (backlog 21 task).
2. Pilih task berlabel `good-first-issue` (TASK-T1-01 … TASK-T2-02) bila baru bergabung.
3. Ikuti prompt master 3 tahap (§4): NotebookLM → OpenCode (draft T1) → Claude (upgrade tier).
4. Preserve 100%: sitasi `(Nama, Tahun)`, istilah Arab italic, heading, substansi.
5. Verifikasi lokal sebelum push (lihat `docs/verifikasi.md`):
   ```powershell
   .\scripts\verify-tier.ps1 -FilePath "<file>" -Tier "T<n>"
   ```
   WARN tetap lolos; FAIL (atau status tak dikenali) harus diperbaiki.
6. CI (`verify-tier`) jalan otomatis tiap push/PR ke `main`; unduh artifact
   `verify-tier-results` bila perlu. Jangan mengarang — data tak ada =
   `TIDAK_TERSEDIA`; konflik dicatat dua versi.
